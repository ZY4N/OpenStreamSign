...
namespace ztu {
	...
	namespace packs {
		template<size_t index, template<typename> class Pred, typename... Ts>
		struct index_of {};

		template<size_t Index, template<typename> class Pred, typename T, typename... Ts>
		struct index_of<Index, Pred, T, Ts...> {
			static constexpr auto index_of_rest = index_of<Index + 1, Pred, Ts...>::value;
			static constexpr auto value = Pred<T>::value ? Index : index_of_rest;
		};

		template<size_t Index, template<typename> class Pred>
		struct index_of<Index, Pred> {
			static constexpr size_t value = Index;
		};
	};

	template<template<typename> class Pred, typename... Ts>
	static constexpr auto index_of = packs::index_of<0, Pred, Ts...>::value;
	...
	namespace packs {
		...
		template<size_t Index, template<typename> class Pred, typename U, typename... Ts>
		struct index_of_last<Index, Pred, U, Ts...> {
			static constexpr auto index_of_rest = index_of_last<Index + 1, Pred, Ts...>::value;
			static constexpr auto value = Pred<U>::value and index_of_rest == -1 ? Index : index_of_rest;
		};

		template<size_t Index, template<typename> class Pred>
		struct index_of_last<Index, Pred> {
			static constexpr size_t value = -1;
		};
	};

	template<template<typename> class Pred, typename... Ts>
	static constexpr auto index_of_last = packs::index_of_last<0, Pred, Ts...>::value;
	...
	struct pack<T, Ts...> {
		...
		template<template<typename> class Pred>
		static constexpr auto index_of = ztu::index_of<Pred, T, Ts...>;

		template<template<typename> class Pred>
		static constexpr auto index_of_last = ztu::index_of_last<Pred, T, Ts...>;
